...
<div class="flex flex-col items-center gap-6">
    <div class="dash-card dash-reveal w-full max-w-lg flex flex-col items-center text-center gap-5 px-8 py-10">
        <span class="flex items-center justify-center size-20 rounded-full bg-sukses/15">
            <svg class="size-10 text-sukses" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        </span>
        <div class="flex flex-col gap-1.5">
            <h1 class="text-2xl font-black text-ink tracking-tight">Pembayaran Berhasil!</h1>
        ...
        <div class="w-full rounded-xl bg-paper-100 p-4 flex flex-col gap-3 items-start text-left">
            <div class="flex items-center justify-between w-full">
                <span class="text-xs font-semibold text-ink-muted">Nomor pembayaran</span>
                <span class="text-xs font-bold text-ink">{{ $nomorPembayaran ?? 'PK-' . now()->format('Ymd') . '-' . strtoupper(substr(md5((string) auth()->id()), 0, 8)) }}</span>
            </div>
            @if ($metodeNama)
                <div class="flex items-center justify-between w-full">
                    <span class="text-xs font-semibold text-ink-muted">Metode</span>
                    <span class="text-xs font-bold text-ink">{{ $metodeNama }}</span>
                </div>
            @endif
            <div class="flex items-center justify-between w-full">
