...
@section('pageContent')
    @php
        ...
        $kosong = $p->kosong ?? $p->jawaban->filter(fn ($j) => $j->pilihan === null)->count();
        $total = $p->total;
        $akurasi = $p->akurasi;
        $skor = $p->skor;
            ...
            <div class="latsol-hasil-hero-info" style="flex:1;min-width:240px;">
                ...
                <p class="latsol-hasil-hero-sub">
                    @if (isset($tuntas) && $tuntas)
                        <b style="color:var(--green-text);">✓ Tuntas</b> — capai {{ $passing ?? 70 }} untuk lanjut bab berikutnya.
                    @else
                        <b style="color:#ff7a7a;">Belum tuntas</b> — butuh minimal {{ $passing ?? 70 }} untuk lanjut bab berikutnya.
                    @endif
                    <span class="dash-pill" style="margin-left:8px;">{{ $gjTeks }}</span>
                    <span style="margin-left:8px;opacity:.8;">Dikerjakan {{ $p->created_at->format('d M Y, H:i') }}.</span>
                </p>
            </div>
        </div>
